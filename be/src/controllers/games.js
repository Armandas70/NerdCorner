...
export const getGames = async (req, res) => {
    ...
    try {
        db.manyOrNone(query)
            .then((data) => {
                console.log('GET Games results:', data);
...
export const getGame = async (req, res) => {
    ...
    if (isNaN(id)) {
        console.log("GET Game error:", `Invalid id`);
    ...
    try {
        db.oneOrNone(query)
            .then((data) => {
                if (data == null) {
                    console.log("GET Game error:", `Game with id ${id} not found`);
                    res.status(404).json({ message: "Game not found" });
                } else {
                    console.log('GET Game results:', data);
                    res.status(200).json(data);
                }
            })
            .catch((error) => {
...
export const deleteGame = async (req, res) => {
    ...
    // Checks if id is a valid number
    if (isNaN(id)) {
        console.log("DELETE Game error:", `Invalid id`);
        res.status(400).json({ message: "Invalid id" });
        return;
    }
    ...
    try {
        ...
        db.oneOrNone(getQuery)
            .then((data) => {
                if (data == null) {
                    console.log("DELETE Game error:", `Game with id ${id} not found`);
                    res.status(404).json({ message: "Game not found" });
                } else {
                    // Tries to delete the game with id (if game exists)
                    db.query(deleteQuery)
                        .then((data) => {
                            console.log('DELETE Game results:', data);
                            res.status(200).json(data);
                        })
                        .catch((error) => {
